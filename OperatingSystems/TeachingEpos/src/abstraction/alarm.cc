...
void Alarm::handler(const IC::Interrupt_Id & i)
{
    ...
    if(Traits<IC>::dispatch_debugged) {
        db<Alarm>(TRC) << "Alarm::handler() _elapsed=" << _elapsed << ", _request.size()=" << _request.size() << endl;
    }
    ...
    if(!_request.empty()) {
        ...
        if(_request.head()->promote() <= 0) { // rank can be negative whenever multiple handlers get created for the same time tick
            ...
            db<Alarm>(TRC) << "Alarm::handler() alarm=" << alarm << ", _times=" << alarm->_times << ", next_tick=" << _request.head()->rank() << endl;
        }
    }
