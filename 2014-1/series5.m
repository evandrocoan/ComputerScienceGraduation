...
%calcular os Bs
A = [
[c(3) c(4) c(5) -c(6)]; %o c(0) foi trocado por c(1) e etc pq o octave começa do 1
[c(4) c(5) c(6) -c(7)];
[c(5) c(6) c(7) -c(8)];
];
fgauss(mpade, A)
...
a(5) = b(3)*c(1+1) + b(2)*c(2+1) + b(1)*c(3+1) + c(4+1);
b
a
%R32 = (a0 + a1x + a2x^2 + a3x^3 + a4x^4)/(1 + b1x + b2*x^2 + b3x^3)
tP = tp;
ypade=(a(0+1) + a(1+1) .*tP + a(2+1) .*tP.^2 + a(3+1) .*tP.^3 + a(4+1) .*tP.^4) ./(1 + b(1) .*tP + b(2) .*tP.^2 + b(3) .*tP.^3);
...
plot(tp, erroInter, "k;erro(x)=|f(x) - Pn(x)|;", tM, erroMac, "m;erro(x)=|f(x) - Mac(x)|;", tC, erroCheb, "g;erro(x)=|f(x) - Cheb(x)|;", tP, erroPade, "r;erro(x)=|f(x) - R43(x)|;");
