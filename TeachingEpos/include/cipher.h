...
/*
   The code below was taken from https://github.com/kokke/tiny-AES128-C
   and adapted to EPOS
...
public:
    ...
    bool crypt(const char * data, const char *key, char *result, bool enc_dec) {
        for(unsigned int i = 0; i < KEY_LENGTH - 1; i++)
            result[i] = data[i] ^ key[i+1];
        result[KEY_LENGTH - 1] = data[KEY_LENGTH - 1] ^ key[0];
        return true;

        bool ret = false;

        db<AES>(TRC) << "AES:: " << (enc_dec ? "en" : "de") << "crypt => data = [" << (int)data[0];
        for(unsigned int i=1;i<KEY_LENGTH;i++)
            db<AES>(TRC) << " ," << (int)data[i];
        db<AES>(TRC) << "]" << endl << "key = [" << (int)key[0];
        for(unsigned int i=1;i<KEY_LENGTH;i++)
            db<AES>(TRC) << " ," << (int)key[i];
        db<AES>(TRC) << "]" << endl;
        db<AES>(TRC) << "mode = " << (_mode == ECB ? "ECB" : "CBC") << endl;
        db<AES>(TRC) << "]" << endl << "iv = [" << (int)iv[0];
        ...
        switch (_mode) {
            case CBC:
                if(enc_dec)
                    AES128_CBC_encrypt_buffer(result, data, KEY_LENGTH, key, iv);
                else
                    AES128_CBC_decrypt_buffer(result, data, KEY_LENGTH, key, iv);
                ret = true;
                break;
            case ECB:
                if(enc_dec)
...
private:
    void AES128_CBC_encrypt_buffer(char* output, const char* _input, int length, const char* key, const char* iv) {
